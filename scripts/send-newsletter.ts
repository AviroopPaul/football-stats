      ...
      <body>
        ...
        ${news.map(item => `
          <div class="news-item">
            ...
            <h2>${item.title}</h2>
            <div class="source">Source: ${item.source.name} | ${format(new Date(item.publishedAt), 'MMM do, h:mm a')}</div>
  ...
  try {
    ...
    for (const subscriber of subscribers) {
      try {
        await sgMail.send({
          to: subscriber.email,
          from: "apavirooppaul10@gmail.com", // Update this with your verified sender
          subject: `FootStats Daily Newsletter - ${format(new Date(), 'MMMM do, yyyy')}`,
          html: emailContent,
        });
