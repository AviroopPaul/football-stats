...
function extractKeywords(text: string): string[] {
  ...
  const keywords = [
    ...
    'Real Madrid', 'Barcelona', 'Bayern Munich', 'PSG', 'UEFA', 'FIFA'
  ];
...
export default function NewsPage() {
  ...
  const [filters, setFilters] = useState<NewsFilters>({ search: '', league: '', country: '' });
  ...
  const filteredNews = news.articles.filter(article => {
    ...
    const leagueMatch = filters.league === '' || 
      content.includes(filters.league.toLowerCase());
    const countryMatch = filters.country === '' || 
      content.includes(filters.country.toLowerCase());
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
      <div className="container mx-auto px-6 py-6">
        ...
        <h1 className="text-4xl font-bold text-white mb-8 text-center">Latest Football News</h1>
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {extractKeywords(article.title + ' ' + article.description).map((keyword, idx) => (
                    <span
                      key={idx}
